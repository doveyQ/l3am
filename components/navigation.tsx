      ...
      <div className="flex h-16 items-center justify-between px-16">
        <div className="flex flex-1 justify-start">
          <Link href="/" className="flex items-center space-x-2">
            <span className="text-2xl font-bold bg-gradient-to-r from-blue-300 to-purple-600 bg-clip-text text-transparent">
              l3am.dev
            </span>
          </Link>
        </div>
        ...
        <div className="flex flex-1 items-center justify-end w-full px-4">
          {mounted && (
            <button
              onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
              className="inline-flex items-center justify-center rounded-md text-xl font-medium transition-colors hover:bg-accent hover:text-accent-foreground h-9 w-9"
